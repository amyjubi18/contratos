      ...
      for (let i = 0, j = characterProperties.length; i < j; i++) {
          ...
          if (attribute === "type") {
              ...
              characterData[attribute] = {
                  ...
                  image: option.dataset.imageUrl || e.target.querySelector("[name='defaultImage']").value, // Usa la imagen por defecto si no hay otra
              };
          }
      }
...
const generatePDF = (characterData, preview) => {
  ...
  // Establece la imagen por defecto aquí
  const defaultImageUrl = "/views/img/user.png";
  ...
  const img = new Image();
    ...
    img.onload = function() {
        ...
        doc.setFontSize(20);
        doc.setFont("helvetica", "bold");
        doc.text("Tipo: Mago", docWidth - 20, 45, { align: "right" });
        doc.line(0, docHeight - 60, docWidth, docHeight - 60);
        doc.text(`Fuerza: `, 10, docHeight - 40);
        doc.text(`Magia: `, 10, docHeight - 30);
        doc.text(`Velocidad: `, 10, docHeight - 20);
